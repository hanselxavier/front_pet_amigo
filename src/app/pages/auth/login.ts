        ...
        <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
          <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
            <div class="text-center mb-8">
              ...
              <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Control de Mascotas</div>
              <span class="text-muted-color font-medium">Inicia sesión para continuar</span>
            </div>

            <div>
              @if (errorMensaje()) {
                <p-message severity="error" [text]="errorMensaje()!" styleClass="w-full mb-6" />
              }
              ...
              <div class="flex items-center justify-between mt-2 mb-8 gap-8">
                <div class="flex items-center">
                  <p-checkbox [(ngModel)]="recordarme" id="rememberme1" binary class="mr-2"></p-checkbox>
...
export class Login {
  ...
  iniciarSesion(): void {
    if (!this.email || !this.password) {
      this.errorMensaje.set('Ingresa tu email y contraseña');
      return;
    }
